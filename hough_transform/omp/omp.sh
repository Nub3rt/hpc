...
#PBS -l select=$nodes:ncpus=$cores:ompthreads=$cores:mem=8gb $placement
...
module load -s GCC/11.2.0 GCCcore/11.2.0
"./$name.out" $total_cores $img_path $theta_multiplier $print_time $print_lines
