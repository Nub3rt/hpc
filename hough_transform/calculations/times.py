...
if len(sys.argv) < 2 or not sys.argv[1] in ['mpi', 'omp']:
  print(f'usage: {sys.argv[0]} [mpi/omp]')
  exit(1)
name = sys.argv[1]
...
for nodes in nodes_values:
    for cores in cores_values:
        for edges in edges_values:
            filename = f'{name}.o.{nodes}x{cores}x{edges}x16'
            path = os.path.join(f'../{name}_out_edges', filename)
...
for nodes in nodes_values:
    for cores in cores_values:
        for tmult in tmult_values:
            filename = f'{name}.o.{nodes}x{cores}x320000x{tmult}'
            path = os.path.join(f'../{name}_out_theta', filename)
